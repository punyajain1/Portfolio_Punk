  ...
  return (
    <div
      className={`flex flex-col items-center gap-1 w-24 p-2 cursor-pointer group ${
        selected ? "bg-black text-white" : "text-black hover:bg-gray-200"
      }`}
      onClick={onClick}
    >
      ...
      <span className={`text-xs text-center font-mono bg-white px-1 border border-transparent ${selected ? "bg-black text-white border-white border-dashed" : "group-hover:border-black"}`}>
        {label}
      </span>
    </div>
  );
